<x-layout>
    ...
    <x-breadcrumbs
        :links="['My Jobs'=> route('my-jobs.index'), $job->title => route('my-jobs.show', ['my_job'=> $job]), 'Job Application-'.$jobApplication->user->name => '#']"
        class="mb-4" />
    ...
    <x-job-seeker-card :jobSeeker="$jobApplication->user->jobSeeker">
        ...
        <div class="flex item-center justify-between text-xs text-slate-500">
            ...
            <div class="flex space-x-2 my-4 items-end">
                ...
                @elseif($jobApplication->status == 'under_review')
                <div>
                    Viewed
                </div>
                @else
                <div>
                    <x-link-button
                        :href="route('job.application.updateState', ['job'=> $job, 'application'=> $jobApplication, 'status' => 'accept'])">
                        Accept
                    </x-link-button>
                </div>
                <div>
                    <x-link-button
                        :href="route('job.application.updateState', ['job'=> $job, 'application'=> $jobApplication, 'status' => 'reject'])">
                        Deny
                    </x-link-button>
                </div>
                @endif
